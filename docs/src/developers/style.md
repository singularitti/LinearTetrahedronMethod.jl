...
!!! info
    The style guide is always a work in progress, and not all LinearTetrahedronMethod code
    follows the rules. When modifying LinearTetrahedronMethod, please fix the style violations
...
`LinearTetrahedronMethod` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/LinearTetrahedronMethod.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `LinearTetrahedronMethod` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `LinearTetrahedronMethod` have
